...
const debug = require('debug')('competition-metadata');
...
function addCompetitionMetadata(options)
{
    ...
    return pluginKit.middleware({
        ...
        before: (files, metalsmith) => {
                ...
                debug('adding generation date');
                metadata._generationDate = new Date();
            },

        each: (competitionName, competition, files, metalsmith) => {
                debug('computing lineup stats for all events in %s competition', competitionName);
                _.forEach(competition.units, (eventName) => {
                    debug('computing lineup stats for %s event in %s competition', eventName, competitionName);
