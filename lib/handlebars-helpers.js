const _ = require('lodash');
const logs = require('./logs').forFilename(__filename);
const moment = require('moment-timezone');
const path = require('path');

const helpers = {
    array: (handlebars, ...rest) => {
        const options = _.last(rest);
        return _.initial(rest);
    },

    'let': (handlebars, context, options) => {
        context = _.clone(context);
        _.assign(context, options.hash);
        return options.fn(context);
    },

    pluralize: (handlebars, singular, plural, selector, options) => {
        const size = (_.isNumber(selector) ? selector : _.size(selector));
        return (1 == size ? singular : plural);
    },

    sanitize: (handlebars, s, options) => {
        const sanitizeHtml = require('sanitize-html');
        return sanitizeHtml(s, {
            allowedTags: []
        });
    },

    sentence: (handlebars, context, options) => {
        const unescape = require('unescape-js');

        const hash = _.defaults({}, options.hash, {
            joiner: ', ',
            terminalJoiner: ', and ',
            dualJoiner: ' and ',
        });

        context = _.castArray(_.defaultTo(context, []));

        logs.debug("joining sentence for %o with settings %o", context, hash);

        const data = (options.data ? handlebars.createFrame(options.data) : options.data);

        const renderings = _.map(context, (value, index, collection) => {
            if (data)
            {
                data.key = index;
                data.index = index;
                data.first = (0 == data.index);
                data.last = (1 == _.size(collection) - data.index);
            }

            return options.fn(value, { data: data });
        });

        logs.debug("renderings are %o", renderings);

        hash.joiner = unescape(hash.joiner);
        hash.terminalJoiner = unescape(hash.terminalJoiner);
        hash.dualJoiner = unescape(hash.dualJoiner);

        const result = (renderings.length < 3 ?
                                _.join(renderings, hash.dualJoiner) :
                                _.join(_.dropRight(renderings), hash.joiner) + hash.terminalJoiner + _.last(renderings));

        logs.debug("result is %o", result);

        return result;
    },

    stringConcat: (handlebars, ...rest) => {
        const options = _.last(rest);
        const args = _.initial(rest);
        return args.join('');
    },
}

const fileHelpers = {
    dirname: (handlebars, pathname, options) => {
        return _.trimEnd(path.dirname(pathname), '/');
    },

    dropExtname: (handlebars, pathname, options) => {
        return path.join(path.dirname(pathname),
                         path.basename(pathname, path.extname(pathname)));
    },

    extname: (handlebars, pathname, options) => {
        return path.extname(pathname);
    },

    join: (handlebars, ...rest) => {
        const options = _.last(rest);
        const args = _.initial(rest);
        return path.join(...args);
    },
}

let defaultTimezone = '';

const dateHelpers = {
    formatDate: (handlebars, format, date, options) => {
        const optionsHash = _.defaults(options.hash, {
            tz: defaultTimezone
        });

        date = moment(date);
        if (optionsHash.tz) {
            date = date.tz(optionsHash.tz);
        }

        return date.format(format);
    }
}

function setDefaultTimezone(timezone) {
    if (!_.isUndefined(timezone)) {
        defaultTimezone = timezone;
    }

    return timezone;
}

function registerHelpers(options)
{
    if (!options.handlebars)
    {
        throw "handlebars option must be specified"
    }

    options = _.assign({
        namespace: ''
    }, options);

    _.forEach(helpers, (value, key) => {
        const helperName = options.namespace + key;
        logs.debug('registering %s', helperName);
        options.handlebars.registerHelper(options.namespace + key, _.partial(value, options.handlebars));
    });

    _.forEach(dateHelpers, (value, key) => {
        const helperName = options.namespace + key;
        logs.debug('registering %s', helperName);
        options.handlebars.registerHelper(options.namespace + key, _.partial(value, options.handlebars));
    });

    _.forEach(fileHelpers, (value, key) => {
        const helperName = options.namespace + key;
        logs.debug('registering %s', helperName);
        options.handlebars.registerHelper(options.namespace + key, _.partial(value, options.handlebars));
    });
}

module.exports = {
    defaultTimezone: setDefaultTimezone,
    helpers: helpers,
    register: registerHelpers,    
};
